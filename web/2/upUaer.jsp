...
<%@ page import="conn.User.*,conn.Journal.*,conn.Post.*" %>
...
<tr>
<td>所在崗位</td><td>
<% Post po=new Post();List<PostInfo> list = po.getpo(usin.getST_department()); for(PostInfo poin:list) {%>
<input type="radio" name="po" id="po"  value="<%=poin.getPO_name() %>" <% if(usin.getST_post().equals(poin.getPO_name())) out.print("checked"); %>/>
<%=poin.getPO_name() %><%} %></td></tr>
<tr>
