...
String po=usin.getST_power();  //获取登录用户的权限，用来查询只能查到比自己权限小的用户
ResultSet rsu;
...
int intPage;
int i;
 Conn conn=new Conn();sql="select * from staff where ST_department ='"+de+"'AND ST_power<"+po;
...
<html>
 ...
 <th class="usertb-th" title="员工家庭住址">员工家庭住址</th><th class="usertb-th" title="员工生日">员工生日</th><th class="usertb-th" title="员工本月工作时长">员工本月工作时长</th>
  <th class="usertb-th" title="员工总工作时长">员工总工作时长</th><th class="usertb-th" title="备注">备注</th>
  <th>操作</th>
  </tr>
  <%
  if(intPageCount>0){
...
//   for(userInfo usi:list) {
  ...
  <% if(intPage>1&&intPage<intPageCount){ %> <a href="./2/updateUser.jsp?page=<%=intPage-1 %>" class="tb-url">上一页</a>
  <a href="./2/updateUser.jsp?page=<%=intPage+1 %>" class="tb-url">下一页</a>
  <% }else if(intPage<=1){ %>上一页  <a href="./2/updateUser.jsp?page=<%=intPage+1 %>" class="tb-url">下一页</a>
  <% }else{ %><a href="./2updateUser.jsp?page=<%=intPage-1 %>" class="tb-url">上一页</a> 下一页<% } %>
  </center>
  </body>
</html>
<% conn.close(); %>
