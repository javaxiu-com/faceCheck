  ...
  if("faceadd".equals(request.getParameter("action")) )
{
 ...
 jo.insert(joi);
 out.print("<script>alert('账户添加成功'); window.location='./insertUser.jsp' </script>");
// response.sendRedirect("user.jsp");  (String)session.getAttribute("usid")
}
else{
out.print("<script>alert('添加失败,请确保面部信息显示在人脸框中，并避免周围人的面部信息显示在人脸框！！！'); window.location='insertuserfa.jsp' </script>");
}
}%>
<%
...
         
 			<form  id="form" name="form"  method="post" action="insertuserfa.jsp?action=faceadd">
